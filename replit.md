...
- June 21, 2025. Guest vs registered user search type defaults implementation: Fixed authentication timing issue where useState initializer ran before user state loaded. Implemented useEffect pattern that sets auth-based defaults only for users without localStorage preferences. Guest users now properly default to "contacts" (70 credits, ~28 seconds) while registered users default to "emails" (240 credits, ~45 seconds). Preserves all existing localStorage preferences and responds correctly to login/logout events. Optimizes guest conversion funnel with 38% faster results
