...
- June 21, 2025. Guest vs registered user search type defaults implementation: Changed default search type for guest users from "emails" (240 credits, ~45 seconds) to "contacts" (70 credits, ~28 seconds) while maintaining "emails" default for registered users. Single-line change in prompt-editor.tsx initialization logic using authentication state: `(user ? 'emails' : 'contacts')`. Preserves localStorage preferences for returning users, optimizes guest conversion funnel with 38% faster results, and maintains full feature access for authenticated users
