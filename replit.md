...
- June 14, 2025. Merge Fields Dialog clickable rows: Made entire merge field rows clickable for direct insertion into form inputs. Added cursor-pointer styling and onClick handler to row divs, with stopPropagation on copy button to prevent conflicts. Rows now insert merge fields at cursor position and auto-close dialog after 500ms, while copy icon remains independent
