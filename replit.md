...
- June 22, 2025. Badge system backward compatibility fix: Added automatic migration logic to hasEarnedBadge() method for Hatchling badge (ID 0). System now checks old notification tracking (notifications[0]) and automatically migrates to new badge system (badges[0]) when needed. Fixes issue where existing users from notification-era didn't have proper badge tracking. Removed unnecessary hasHatchlingBadge field from API responses. New users work correctly from start, legacy users get seamless migration
