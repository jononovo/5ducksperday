...
- July 24, 2025. **Sender Name Modal Loop Fix**: Fixed critical race condition in sender name dialog that caused modal to reappear immediately after saving. Root cause was useEffect dependency on `showSenderNameDialog` creating a trigger loop when combined with cache invalidation timing. Removed `showSenderNameDialog` from useEffect dependencies array, added session-based protection flag `senderNameDialogShownForSession` to prevent multiple displays, reduced user preferences cache staleTime from 5 minutes to 30 seconds for faster updates, and added explicit modal close in onSuccess callback. Fixed timing race where cache invalidation was slower than modal state changes, causing condition `!userPreferences.senderName` to re-trigger before data persisted. Files modified: `client/src/pages/outreach.tsx`.
