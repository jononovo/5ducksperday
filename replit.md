...
- June 19, 2025. Complete Easter egg system implementation: Added scalable hidden bonus system with array-based tracking and case-insensitive triggers. Implemented two easter eggs: "5ducks" (1000 credits) and "Free Palestine" (3000 credits). Frontend intercepts easter egg queries in search input before normal search execution, backend validates triggers case-insensitively, and system prevents duplicate claims through persistent array tracking [0,1] format. Added dedicated `/api/credits/easter-egg` endpoint with secure validation and automatic credit refresh in UI
