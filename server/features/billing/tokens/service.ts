...
export class TokenService {
  ...
  /**
   * Migrate tokens from Replit DB to PostgreSQL
   * This is a one-time migration helper
   */
  static async migrateFromReplitDB(userId: number): Promise<boolean> {
    try {
      console.log(`[TokenService] Attempting to migrate tokens for user ${userId} from Replit DB to PostgreSQL`);
      
      // First check if tokens already exist in PostgreSQL
      const postgresTokens = await storage.getOAuthToken(userId, 'gmail');
      if (postgresTokens) {
        console.log(`[TokenService] Tokens already exist in PostgreSQL for user ${userId}, skipping migration`);
        return true;
      }

      // Try to get tokens from Replit DB (if available)
      try {
        const Database = await import('@replit/database').then(m => m.default);
        ...
        if (tokensData) {
          ...
          const oldTokens: UserTokens = typeof rawData === 'string' ? JSON.parse(rawData) : rawData;
          
          // Save to PostgreSQL
          await this.saveUserTokens(userId, oldTokens);
          
          // Optionally delete from Replit DB after successful migration
          await db.delete(key);
          
          console.log(`[TokenService] Successfully migrated tokens for user ${userId} from Replit DB to PostgreSQL`);
          return true;
        }
      } catch (replitDbError) {
        console.log(`[TokenService] Could not access Replit DB for migration:`, replitDbError);
      }
      
      console.log(`[TokenService] No tokens to migrate for user ${userId}`);
      return false;
    } catch (error) {
      console.error(`[TokenService] Error during migration for user ${userId}:`, error);
      return false;
    }
  }

  /**
   * Migrate all user tokens from Replit DB to PostgreSQL
   */
  static async migrateAllTokens(): Promise<void> {
    try {
      console.log(`[TokenService] Starting bulk migration from Replit DB to PostgreSQL`);
      
      const Database = await import('@replit/database').then(m => m.default);
      const db = new Database();
      
      // Get all keys that match the token pattern
      const allKeysResult = await db.list('user_tokens:');
      
      // Handle Replit DB response format
      const allKeys = Array.isArray(allKeysResult) ? allKeysResult : 
                      (allKeysResult && 'value' in allKeysResult && Array.isArray(allKeysResult.value)) ? allKeysResult.value : [];
      
      console.log(`[TokenService] Found ${allKeys.length} token entries to migrate`);
      ...
      for (const key of allKeys) {
        try {
          // Extract userId from key (format: "user_tokens:123")
          const userId = parseInt(key.replace('user_tokens:', ''));
          if (isNaN(userId)) {
            console.warn(`[TokenService] Skipping invalid key: ${key}`);
            continue;
          }
          
          const success = await this.migrateFromReplitDB(userId);
          if (success) {
            successCount++;
          } else {
            failCount++;
          ...
          console.error(`[TokenService] Error migrating key ${key}:`, error);
          failCount++;
        }
      }
      
      console.log(`[TokenService] Migration complete: ${successCount} succeeded, ${failCount} failed`);
    } catch (error) {
      console.error(`[TokenService] Error during bulk migration:`, error);
    }
  }
}